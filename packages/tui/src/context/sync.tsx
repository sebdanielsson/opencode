    ...
    const result = {
      ...
      session: {
        ...
        addOptimisticPrompt(input: {
          ...
          const match = messages ? search(messages, input.messageID, (message) => message.id) : undefined
          const info: Message = {
            id: input.messageID,
        ...
        removeOptimisticPrompt(sessionID: string, messageID: string) {
          ...
          const messages = store.message[sessionID]
          const match = messages ? search(messages, messageID, (message) => message.id) : undefined
          batch(() => {
            if (match?.found) {
              setStore(
                "message",
            ...
            setStore(
              produce((draft) => {
                ...
                for (const message of visible) {
                  ...
                  if (!fetchedParts) {
                    if (optimisticMessages.has(message.id)) {
                      draft.part[message.id] = currentParts
                      continue
                    }
